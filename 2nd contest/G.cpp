...
std::vector<bool> GetCliques(const Matrix<bool>& adj_matrix,
                             const std::vector<int64_t>& adj_list, int64_t part) {
  int64_t max_mask = 0;
  int offset = 0;
  if (part == 0) {
...
std::vector<uint32_t> MaxConnectedMaskInSecond(const Matrix<bool>& adj_matrix, const std::vector<int64_t>& adj_list,
...
int64_t CountCliques(const Matrix<bool>& adj_matrix) {
  ...
  // std::vector<bool> is_clique_2 = GetCliques(adj_matrix_2, adj_list_2, 2);

  std::vector<int64_t> subcliques_count_2 = HowManySubmasksAreCliques(adj_matrix_2, adj_list_2);

  std::vector<uint32_t> adj_cliques_1 = MaxConnectedMaskInSecond(adj_matrix_1, adj_list_1, adj_list);
  ...
  for (uint32_t mask = 0; mask <= max_mask; ++mask) {
    if (is_clique_1[mask]) {
      //std::cout << mask << " | " << adj_cliques_1[mask] << " | " << subcliques_count_2[adj_cliques_1[mask]] << '\n';
      result += subcliques_count_2[adj_cliques_1[mask]];
    }
  }

  // OutputMatrix(adj_matrix_1);

  // for (auto& vertex : adj_list_1) {
  //   std::cout << vertex << ' ';
  // }
  // std::cout << '\n';

  // OutputMatrix(adj_matrix_2);

  // for (auto& vertex : adj_list_2) {
  //   std::cout << vertex << ' ';
  // }
  // std::cout << '\n';

  // for (int64_t i = 0; i < adj_cliques_1.size(); ++i) {
  //   std::cout << i << '|' << adj_cliques_1[i] << ' ';
  // }
  // std::cout << '\n';

  // for (int64_t i = 0; i < subcliques_count_2.size(); ++i) {
  //   std::cout << i << '|' << subcliques_count_2[i] << ' ';
  // }
  // std::cout << '\n';
